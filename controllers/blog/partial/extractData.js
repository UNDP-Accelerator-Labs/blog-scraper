const { evaluateArticleType, extractLanguageFromUrl, checkSearchTerm, getDocumentMeta } = include("services/");
...
const extractDataFromUrl = async (driver, url) => {
  ...
  try {
    ...
    data.html_content = await driver.getPageSource();
...
if (data.article_type === "document") {
    try {
      if (url.includes(".pdf") || url.includes(".PDF")) {
        ...
        data.postedDate = new Date(pdfContent?.metadata?._metadata["xmp:createdate"]) || null;
        data.postedDateStr = pdfContent?.metadata?._metadata["xmp:createdate"] || null;
        data.articleTitle = pdfContent?.metadata?._metadata["dc:title"] || pdfContent?.text?.substring(0, 100) || null;
   ...
   else // Extract article metadata based on type
   if (data.article_type === "project") {
     try {
       ...
       data.postedDateStr =
         (await driver
           .findElement(
             By.css(
               config[
                 "posted_date_str.element.project_page.css_selector"
               ]
             )
           )
           .getText()) || null;
       ...
       const archorTags = await driver.findElements(
         By.css(
           config[
             "content_url_list.elements.project_page.css_selector"
           ]
         )
       );
         ...
         if (!hasFeaturedClass) {
           ...
           try {
             href = await archorTags[i].getAttribute(
               config[
                 "content_url.element.project_page.attribute"
               ]
             );
           } catch (err) {
             href = "";
    ...
    if (data.article_type === "publications") {
      try {
        ...
        data.postedDateStr =
          (await driver
            .findElement(
              By.css(
                config[
                  "posted_date_str.element.publication.css_selector"
                ]
              )
            )
            .getText()) || null;
        ...
        data.country =
          (await driver
            .findElement(
              By.css(
                config[
                  "country_name.element.publication.css_selector"
                ]
              )
            )
            .getText()) || null;
     ...
     if (url.includes(".medium.com")) {
      ...
      if (config.article_types.filter((p) => url.includes(p)).length > 0) {
        try {
          data.articleTitle =
            (await driver
          ...
          data.country =
            (await driver
              .findElement(
                By.css(
                  config["country_name.element.blog.css_selector"]
                )
              )
              .getText()) || null;
